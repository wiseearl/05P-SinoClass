...
DEFAULT_CONFIG_CANDIDATES = (
	"config-req-rerank.txt",
	"../Index/config-req-rerank.txt",
)
...
def resolve_output_path(script_dir: Path, config: dict[str, str]) -> Path:
	return resolve_relative_path(script_dir, config.get("output"), "output-rerank.txt")
...
def rerank_level2(
	...
	collection_name: str,
	question_embedding: list[float],
	level1_hits: list[dict[str, Any]],
	level2_top_k: int,
) -> list[dict[str, Any]]:
	collection = client.get_collection(name=collection_name)
	chunk_ids = [hit["chunk_id"] for hit in level1_hits if hit.get("chunk_id")]
	if not chunk_ids:
		raise RuntimeError("No chunk ids were returned from level1 retrieval.")

	result = collection.get(ids=chunk_ids, include=["embeddings"])
	raw_ids = result.get("ids", [])
	raw_embeddings = result.get("embeddings", [])
	if len(raw_embeddings) == 0:
		raise RuntimeError("Embeddings are missing from the vector database; rerank cannot continue.")

	embedding_lookup: dict[str, list[float]] = {}
	for index, chunk_id in enumerate(raw_ids):
		vector = raw_embeddings[index] if index < len(raw_embeddings) else None
		if vector is None:
			continue
		embedding_lookup[str(chunk_id)] = list(vector)
	...
	for hit in level1_hits:
		...
		reranked_hit["rerank_score"] = cosine_similarity(question_embedding, chunk_embedding)
		reranked_hits.append(reranked_hit)

	if not reranked_hits:
		raise RuntimeError("No level2 rerank results were produced.")

	reranked_hits.sort(
		key=lambda item: (item["rerank_score"], -(item.get("distance") or 0.0)),
		reverse=True,
	)

	for index, hit in enumerate(reranked_hits[:level2_top_k], start=1):
		hit["level2_rank"] = index

	return reranked_hits[:level2_top_k]
...
def build_prompt(question: str, hits: list[dict[str, Any]]) -> str:
	...
	for hit in hits:
		...
		context_blocks.append(
			f"[Context {hit['level2_rank']}] chunk_id={hit['chunk_id']}, file={file_name}, page={page}, chapter={chapter_name}, rerank_score={hit['rerank_score']:.6f}\n{hit['document']}"
		)
...
def format_level1_hits(hits: list[dict[str, Any]]) -> str:
	blocks: list[str] = []
	for hit in hits:
		metadata = hit["metadata"]
		chapter = hit["chapter"]
		...
		blocks.append(
			f"[{hit['level1_rank']}] chunk_id={hit['chunk_id']}, distance={distance_text}, file={metadata.get('file_name', 'unknown')}, page={metadata.get('page', '?')}\n"
...
def format_level2_hits(hits: list[dict[str, Any]]) -> str:
	blocks: list[str] = []
	for hit in hits:
		metadata = hit["metadata"]
		chapter = hit["chapter"]
		...
		distance_text = f"{distance:.6f}" if isinstance(distance, float) else "N/A"
		blocks.append(
			f"[{hit['level2_rank']}] chunk_id={hit['chunk_id']}, rerank_score={hit['rerank_score']:.6f}, level1_rank={hit['level1_rank']}, level1_distance={distance_text}, file={metadata.get('file_name', 'unknown')}, page={metadata.get('page', '?')}\n"
			f"章節: {chapter.get('chapter', '未對應章節')}\n"
			f"內容:\n{hit['document']}"
		)
	return "\n\n".join(blocks)
...
def write_output(
	...
	content = (
		...
		f"{format_level1_hits(level1_hits)}\n\n"
		"RAG Level 2 Top 4 向量比對結果:\n"
		f"{format_level2_hits(level2_hits)}\n\n"
		"JSON:\n"
...
def main() -> int:
	try:
		...
		level1_top_k = get_int(config.get("level1_top_k"), 20)
		level2_top_k = get_int(config.get("level2_top_k"), 4)
		if level1_top_k < level2_top_k:
			raise ValueError("level1_top_k must be greater than or equal to level2_top_k")
		...
		level2_hits = rerank_level2(
			client,
			collection_name=collection_name,
			question_embedding=question_embedding,
			level1_hits=level1_hits,
			level2_top_k=level2_top_k,
		)
		prompt = build_prompt(question, level2_hits)
		...
		payload = build_payload(
			...
			answer=answer,
			level1_hits=level1_hits,
			level2_hits=level2_hits,
			base_url=base_url,
			model=generation_model,
			embedding_model=embedding_model,
		)
		write_output(output_path, question, answer, level1_hits, level2_hits, payload)
		...
		print(f"Level1 hits: {len(level1_hits)}")
		print(f"Level2 hits: {len(level2_hits)}")
