...
建立一支可讀取 ApikeyHidding/config-apikeyhidding.txt 的 Python 程式 apikeyhidding.py，使用 keyfile 指向的 Ollama API key 串接雲端 Ollama，送出 request="Hello"，並將回覆寫入 output=./output.txt。
...
執行完成後，回覆內容會寫入 ApikeyHidding/output.txt。
...
- 自動選擇模型 devstral-small-2:24b
- 成功取得 Hello 的回覆並寫入 ApikeyHidding/output.txt